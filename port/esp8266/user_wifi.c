...
void user_setup_wifi(void) {
    ...
    if (saved_ssid_len > 0) {
        PORT_PRINTF("There is a saved WLAN STA config");
        /* Set station mode */
        ...
        PORT_PRINTF("There is no saved WLAN STA config");
...
int user_wifi_format_ssid_str(char *ssid_buffer) {
    ...
    strncpy(ssid_buffer, MIST_CONFIG_SOFTAP_SSID_PREFIX, ssid_prefix_len);
    ...
    strncpy(ssid_buffer + ssid_prefix_len,
    ...
    os_sprintf(ssid_buffer + ssid_prefix_len + ssid_customer_str_len,
        "%02x%02x%02x", ap_mac_addr[3], ap_mac_addr[4], ap_mac_addr[5]);
    
    int ssid_len = strnlen(ssid_buffer, SSID_NAME_MAX_LEN);
    ssid_buffer[SSID_NAME_MAX_LEN-1]=0;    /* Ensure null termination in any
    case */
    return ssid_len;
}
...
void user_wifi_start_ap(void) {
    ...
    int ssid_len = user_wifi_format_ssid_str(ssid);
    PORT_PRINTF("SSID is %s", ssid);
    ...
    if (!wifi_set_broadcast_if(2)) {
        PORT_PRINTF("Could not set UDP bcast if to softap\n");
    }
...
void ap_scan_done_cb(void *arg, STATUS status) {
    if (status == OK) {
        ...
        PORT_PRINTF("AP scan: Status not OK, what does that mean?");
    }
    if (user_wifi_get_mode() == USER_WIFI_MODE_SETUP) {
        user_wifi_setup_commissioning();
    }
    else {
        PORT_PRINTF("Nothing to do, just idling?");
    }
}
